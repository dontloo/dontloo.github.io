...
For \\(n\\) i.i.d variables with zero mean, as \\(n\\) gets larger, its mean gets to zero (the law of large numbers) and its sum goes to infinity.
...
for a smaller scaling factor the term will blow up to positive/negative infinity, only for \\( \sqrt{n} \\) it converges to a distribution.
