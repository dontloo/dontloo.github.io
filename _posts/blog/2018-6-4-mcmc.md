...
\end{matrix}
\\]
has a unique eigenvector (\\(\pi=[0.5, 0.5]\\)) associated to the eigenvalue 1. Say a frog start at the left (L) lily, in the next step it will definitely jump to the right (R) lily according to the matrix, after another step, it will definitely jump to the left, actually it will keep alternating between two states and never reaches a stationary distribution.
...
\\[\pi = \lim_{n\rightarrow \inf} P(X_n|X_0). \\]
