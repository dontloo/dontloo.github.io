...
Now we know a Markov chain has a unique stationary distribution \\(\pi\\) if there's one closed positive recurrent communicating class, one may wonder how does a Markoc chain behave as \\(n\to\infty\\). There're two limiting behaviours people usually care about, we will check the relavent coditions and see how they're applied to practical problems.

The first behaviour is that the sample average converges to the stationary distribution as \\(n\to\infty\\)
\\[\lim_{n\to\infty} \frac{1}{n} \sum^{n}_{m=1} I\\{x_m=i\\}=\pi_i \text{ WP1}\\]
where \\(x_m ~ P(X_m|X_0)\\) are samples from the Markov chain.
...
\end{matrix}
\\]
has only one eigenvector \\(\pi=[0.5, 0.5]\\) associated to the eigenvalue 1. Say a frog start at the left lily, in the next step it will definitely jump to the right lily according to the matrix, after another step, it will definitely jump to the left and so forth. 
...
It turns out convergence without averaging is a stronger condition than convergence with averaging. The first property holds true as long as there's a unique stationary distribution, to go from the first to the second, a further condition known as aperiodicity is needed on the chain. 

For more about limiting distribution for Markov chains you can refer to [this tutorial by Karl Sigman](http://www.columbia.edu/~ks20/stochastic-I/stochastic-I-MCII.pdf), also some discussions [here](https://math.stackexchange.com/questions/2812442/what-is-the-necessary-and-sufficient-condition-of-a-markov-chain-having-a-limiti) and [here](https://math.stackexchange.com/questions/2816677/what-is-the-necessary-and-sufficient-condition-of-markov-chain-sample-average-co).
...
### Ergodicity
Another commonly mentioned property is called ergodicity, which is essentially being irreducible and having a limiting distribution, more specifically,
- for a finite MC it holds that, aperiodic + irreducible ⇔ ergodic
...
The Monte Carlo method is basically to estimate expected values using samples, 
...
A related by slightly different task is to generate samples from a given distribution, which is called sampling. If we can do sampling then we are able to estimate the expectation (Monte Carlo) for sure, in some cases we can estimate the expectation without the need for precise sampling.

### Sampling Wheel

Both tasks are not easy for an arbitrary given distribution especially on high dimensional space, we're going to see how to use Markov chains for Monte Carlo estimation and sampling a.k.a Markov chain Monte Carlo (MCMC).

### MCMC
The central idea of MCMC is to construct a Markov chain whose unique stationary distribution is the desired distribution \\(\pi\\), then from previous discussion of limiting behaviours we know
\\[\lim_{n\to\infty} \frac{1}{n} \sum^{n}_{m=1} I\\{x_m=i\\}=\pi_i \text{ WP1}\\]
which implies
\\[E_\pi[f(x)] \approx \frac{1}{N}\sum^M_{i=1}f(x_i)\\]
for any bounded function \\(f\\). 

Further if \\(\pi\\) is a limiting distribution,
\\[\pi = \lim_{n\to\infty} P(X_n|X_0) \\]
then we'll be able to draw samples from \(\pi\\) after the chain converges.

We've already discussed how to check the existance of a unique stationary distribution and a limiting distribution, let's see how to construct a Markov chain such that \\(\pi\\) is a stationary distribution and transition probability should be easy to sample from.
### Giibs Sampling
 
 ### Metropolis Hastings
 **Detailed Balance**
 
 adding constants to diagonals doesn't change eigenvalues
