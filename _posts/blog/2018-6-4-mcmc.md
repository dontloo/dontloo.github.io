...
where \\(x_m \sim p(X_m|X_0)\\) are samples from the Markov chain.
...
The central idea of MCMC is to construct a Markov chain whose unique stationary distribution is the target distribution \\(\pi\\), then from previous discussion of limiting behaviours we know  
...
We've already discussed how to check the existance of a unique stationary distribution and a limiting distribution, now let's see how to construct a Markov chain such that \\(\pi\\) is a stationary distribution and transition probability should be easy to sample from.
...
is a sufficient but not necessary condition for a distibution to be staionary. To satisfy the detailed balance condition we want the transition probabilities between any two states to be in proportion to the stationary distribution
\\[\frac{p(X_{n+1}=j|X_{n}=i)}{p(X_{n+1}=i|X_{n}=j)}=\frac{\pi(X_{n}=j)}{\pi(X_{n}=i)}.\\]
A simple solution is just letting the transition probability equal to the the stationary distribution itself
\\[p(X_{n+1}=j|X_{n}=i)=\pi(X_{n}=j), p(X_{n+1}=i|X_{n}=j)=\pi(X_{n}=i).\\]
A better solution would be
\\[p(X_{n+1}=j|X_{n}=i)=\min \(1, \frac{\pi(X_{n}=j)}{\pi(X_{n}=i)} \).\\]
Sines \\(\pi\\) only appears as a ratio here, it allows \\(\pi\\) to be an unnormalized distribution. This could be very useful when the normalizer is hard to compute.

The Metropolis Hastings algorithm 
adding constants to diagonals doesn't change eigenvalues

### Irreduciblily and Ergodicity
