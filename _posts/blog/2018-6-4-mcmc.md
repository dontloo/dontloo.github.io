...
### MCMC
The central idea of MCMC is to construct a Markov chain whose unique stationary (or limiting) distribution is the desired distribution \\(\pi\\), and at the same time the trasition probability \\(P(X'|X)\\) should be easy to sample from. 
