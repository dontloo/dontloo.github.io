...
The limit is zero for any state distributions for limiting distributions. In the above example, the state distribution alternates between \\([1,0]\\) and \\([0,1]\\) the KL divergence \\(D_{KL}(P_n|\pi)\\), though not increasing, is always \\(\log 2\\).

### Ergodicity
A Markov chain is called ergodic if the limiting distribution exists. The conditions for ergodicity are somewhat less intuitive, we only give some sketches here.
...
For more details you can take a look at [18.2](https://www.youtube.com/watch?v=ZjrJpkD3o1w), [18.3](https://www.youtube.com/watch?v=tByUQbJdt14), [18.4](https://www.youtube.com/watch?v=Pce7KKeUf5w) and [18.5](https://www.youtube.com/watch?v=daY4lgEyEPc) of the ML video series by mathematicalmonk, and [this tutorial by Karl Sigman](http://www.columbia.edu/~ks20/stochastic-I/stochastic-I-MCII.pdf).

## Monte Carlo
Simply put, the Monte Carlo method is to estimate expected values by using samples, it were central to the simulations required for the Manhattan Project during the war, and we're going to introduce how to use Markov chains for Monte Carlo, which is abbreviated as MCMC.
...
Stationary distributions, Irreducibility, and Aperiodicity   
