...
where \\(\{x_m\}\\) are idependent samples from \\(p(x)\\).

If we do not care much about how the samples are drawn, the samples only have to satisfy a weaker condition
\\[\frac{1}{M} \sum^{M}_{m=1} I\\{x_m=i\\}=p(x_m=i)\\]
