...
We already discussed how to check the existance of a unique stationary distribution and a limiting distribution, let's see how to construct a Markov chain such that \\(\pi\\) is a stationary distribution and transition probability should be easy to sample from.
### Giibs Sampling
