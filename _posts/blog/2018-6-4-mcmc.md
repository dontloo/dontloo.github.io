...
Still sampling directly form \\(\pi\\) is hard, unlike Gibbs sampling, where the transition proability is separated into one dimensional conditional probabilities, the Metropolis Hastings algorithm decomposes the transition proability into a proposal distribution \\(q\\) (easy to sample from) and an acceptance proability \\(\alpha\\) (to satisfy detailed balance)
\\[\pi(X_{n}=i)q(X_{n+1}=j|X_{n}=i)\alpha(X_{n+1}=j|X_{n}=i)=\pi(X_{n}=j)q(X_{n+1}=i|X_{n}=j)\alpha(X_{n+1}=i|X_{n}=j) \forall i,j \\]
Say at time \\(n\\) we're in state \\(x_n)\\), then we draw a sample \\(x_{n+1}')\\) from the proposal distribution \\(q(X_{n+1}|X_{n})\\) and then accept it with probability \\(\alpha(X_{n+1}|X_{n})\\). 
...
Since its value has to be between 0 and 1, a solution would be
