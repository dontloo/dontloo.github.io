...
The second behaviour to check is the sample average converges to the expectation w.r.t. the stationary distribution as \\(n\to\infty\\)
\\[P(lim_{n\to\infty} \frac{1}{n}\sum^{n-1}_{k=0}f(x_k)=\bar{f})=1\\]
where \\(x_k ~ P(X_k|X_0)\\) are samples from the Markov chain, \\(f:I\rightarrow R\\) is any bounded function, \\(\pi\\) is the unique stationary distribution and \\(\bar{f}=\sum_{i\in I}\pi_if_i\\).

Let's illustrate the difference between these two limiting behavious by the following example. The matrix
\\[
\begin{matrix}
    0 & 1 \\\
    1 & 0 \\\
