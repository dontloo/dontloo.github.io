...
Suppose we have a three dimensional distribution \\(p(x1, x2, x3)\\), and we're in state \\((x1_{n}, x2_{n}, x3_{n})\\) at time \\(n\\), we get to the next state following the strategy
