...
\\[\pi(X_{n}=i)q(X_{n+1}=j|X_{n}=i)\alpha(X_{n+1}=j|X_{n}=i)=\pi(X_{n}=j)q(X_{n+1}=i|X_{n}=j)\alpha(X_{n+1}=i|X_{n}=j) \\]
Say at time \\(n\\) we're in state \\(x_n\\), then we draw a sample \\(x_{n+1}'\\) from the proposal distribution \\(q(X_{n+1}|X_{n})\\) and then accept it with probability \\(\alpha(X_{n+1}|X_{n})\\). Thus the transition probability becomes
