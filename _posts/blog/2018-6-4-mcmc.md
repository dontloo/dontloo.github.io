...
## Limiting Behaviours
Now we know a Markov chain has a unique stationary distribution \\(\pi\\) if there's one closed communicating class, one may wonder what is the limiting behaviour as \\(n\to\infity\\). There're two limiting behaviours people usually care about, we will discuss relavent coditions and see how they're applied to practical problems.

First if the state distribution always converges to the stationary distribution for any initial probability \\(X_0\\), then \\(\pi\\) is called the [limiting distribution](https://www.probabilitycourse.com/chapter11/11_2_6_stationary_and_limiting_distributions.php)
...
The second behaviour to check is the sample average approaches the expectation w.r.t. the stationary distribution as \\(n\to\infity\\)
\\[P(lim_{n\to\infty} \frac{1}{n}\sum^{n-1}_{k=0}f(x_k)=\bar{f})=1\\]
where \\(x_k ~ P(X_k|X_0)\\) are sampels from the Markov chain, \\(f:I\rightarrow R\\) is any bounded function, \\(\pi\\) is the unique stationary distribution and \\(\bar{f}=\sum_{i\in I}\pi_if_i\\).
