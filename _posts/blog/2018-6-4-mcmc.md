...
It turns out convergence without averaging is a stronger condition than convergence with averaging. The first property holds true as long as there's a unique stationary distribution, to go from the first to the second, a further condition known as aperiodicity is needed on the chain. For more about limiting distribution for a Markov chains you can refer to [this tutorial by Karl Sigman](http://www.columbia.edu/~ks20/stochastic-I/stochastic-I-MCII.pdf).
