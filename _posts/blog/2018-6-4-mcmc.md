...
\\[P(X_{n+1}=L|X_n=L)=0.3, P(X_{n+1}=R|X_n=L)=0.7, P(X_\{n+1\}=L|X_n=R)=0.5, P(X_\{n+1\}=R|X_n=R)=0.5.\\]
...
If a distribution \\(\pi\\) remains unchanged in the Markov chain it is called a stationary distribution, formally if follows
\\[\pi(X=i)=\sum_jP(X=1|X=j)\pi(X=j).\\]
