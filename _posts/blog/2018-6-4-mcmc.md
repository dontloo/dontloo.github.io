...
A sloppy sketch of the proof goes as follows. The transition matrix can be decomposed as \\(P = V\Lambda V^{—1}\\), where \\(V\\) is a matrix of eigenvectors and \\(\Lambda\\) is a diagnal matrix of eigenvalues, we know \\(P^n =V\Lambda^n V^{—1} \\) (ref: [Diagonalizing a Matrix](https://www.youtube.com/watch?v=U8R54zOTVLw)). Then we decompose the initial distribution as a linear combinition of the eigenvectors \\(X_0=VQ\\) where \\(Q\\) is a diagonal matrix. Then state distribution at time \\(n\\) is 
\\[X_n = P^nX_0 =V\Lambda^n V^{—1}VQ = V\Lambda^n Q.\\]
Since the largest eigenvalue in \\(\Lambda\\) is one and others have magnitude smaller than \\(1\\), the rest elements in \\(\Lambda^n\\) will vanish as \\(n\to\infty\\). So \\(X_n\\) will converge to the corresponding eigenvector in \\(V\\), and the difference between \\(X_n\\) and the stationary distribution \\(\pi\\) is in proportional to \\(|\lambda_2|^n\\), where \\(\lambda_2\\) is the second large eigenvalue in magnitude. 
