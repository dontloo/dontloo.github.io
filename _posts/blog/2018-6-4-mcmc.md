...
\end{matrix}
\\]
there's one closed communicating class consisting of only one state \\(C=\\{1\\}\\), and \\(\pi=[0, 1]\\) is the only stationary vector.
...
Here's a [nice tutorial](http://wwwf.imperial.ac.uk/~ejm/M3S4/NOTES3.pdf) for reference.
