...
Now we know a Markov chain has a unique stationary distribution if there's one closed communicating class, can we always converge to the stationary distribution strating from any initial state?
...
If a Markov chain always converges to the stationary distribution \\(\pi\\) for any initial probability \\(X_0\\), then \\(\pi\\) is called the [limiting distribution](https://www.probabilitycourse.com/chapter11/11_2_6_stationary_and_limiting_distributions.php)
\\[\pi = \lim_{n\to\infty} P(X_n|X_0). \\]

### Ergodicity
The ergodicity property of Markov chains guarantees the limiting distribution exists. The definition of ergodicity is less intuitive, we only give some sketches here.
