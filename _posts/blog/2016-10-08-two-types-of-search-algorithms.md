...
For hash based partitions, the time complexity of getting the belonging cell of a query is constant w.r.t the number of cells \\(N\\). However the time complexity of getting the adjacent cells could be much higher depending on the choice of hash functions. There are many options available, such as locality sensitive hashing (LSH), k-means, hierarchical k-means, product quantization, which are not going to be discussed in this blog.
