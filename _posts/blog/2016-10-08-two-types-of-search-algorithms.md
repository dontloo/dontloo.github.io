...
For tree based partitions, the running time of getting the belonging and adjacent cells can be bounded by (\\(O(\log N)\\)) with the use of priority queues. There's a very nice tree based algorithm named [ANNOY (Approximate Nearest Neighbors Oh Yeah)](https://erikbern.com/2015/10/01/nearest-neighbors-and-vector-models-part-2-how-to-search-in-high-dimensional-spaces/) that has basically every merit I ever wanted for ANN search, good job.
