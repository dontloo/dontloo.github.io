...
Because \\(\mathbf{h}\\) does not have a unique solution, since it has one more degree of freedom than \\(\mathbf{x}\\) (sort of analogous to the biased variance estimation because we don't know where is the ture mean \\(\mu\\)). Similarly the scatter matrices of Linear Discriminative Analysis (LDA) mentioned in the paper can be thought of as another solution for \\(\mathbf{h}\\) based on \\(\mathbf{x}\\) directly. Whereas doing EM has turned out to be a better choice.
