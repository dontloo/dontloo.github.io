...
\\[= F(q_x, \theta) + D_{KL}(q_{z\|x} \| p_{z\|x})\\]
...
In the E-step we optimize \\(F(q_x, \theta)\\) w.r.t \\(q_x\\) while holding \\(\theta\\) fixed. Since \\(\log p(x\|\theta)\\) does not depend on \\(q_x\\), the largest value of \\(F(q_x, \theta)\\) occurs when \\(D_{KL}(q_{z\|x} \| p_{z\|x})=0\\), we have again \\(q_x(z) = p(z\|x,\theta)\\). In the M-step \\(F(q_x, \theta)\\) is maximized w.r.t \\(\theta\\), which is the same as the above section.
...
\\[\sum_x D_{KL}(q_{z\|x} \| p_{z\|x}) = - \sum_x \sum_z q_x(z) \log \frac{p(z\|x,\theta)}{ q_x(z) }\\]
