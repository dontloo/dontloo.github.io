...
\\[\sum_x F(q_x, \theta) = \sum_x \sum_{z q_x(z) \log \frac{p(x,z\|\theta)}{q_x(z)}\\]
If we let \\(q(x,z) = q(z\|x)p(x)\\), where \\(q(z\|x) = q_x(z)\\) and \\(p(x)=\frac{1}{\|X\|}\sum_{i}\delta_i(x)\\) is a distribution that place all its mass on the observed data \\(X \\), then we have
