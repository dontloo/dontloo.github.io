...
\\[\sum_x p(x)f(x) dx = \|X\| sum_x f(x). \\]
Then the formula can be rewritten as 
\\[\frac{\|X\|}{\log\|X\|} \int_x \sum_z q(x,z) \log \frac{p(x,z\|\theta)}{ q(z\|x) } dx  = -\frac{\|X\|}{\log\|X\|} D_{KL}(q(x,z) \| p(x,z\|\theta)). \\]
Therefore the E-step is minimizing \\(D_{KL}(q(x,z) \| p(x,z\|\theta))\\) w.r.t \\(p_\theta\\).

Similarly for the \\(D_{KL}(q_x(z) \| p(z\|x,\theta))\\) term, it can be rewritten as 
\\[- \sum_z q(z) \log \frac{p(z\|x,\theta)}{q(z)} = - \sum_i \sum_z q_i(z) \log \frac{p(z\|x,\theta)}{ q_i(z) }\\]
