...
At the E-step, we find the \\(q\\) that maximizes this lower bound while keeping \\(\theta\\) fixed. Since \\(q\\) has to satisfy the properties of being a probability distribution, the problem becomes, for each data point \\(x\\),
...
At the M-step we keep \\(q_x(z)\\) fixed and maximize over \\(\theta\\).
