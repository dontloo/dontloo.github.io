...
As shown in the LDA paper, the distribution of \\(\mu\\) is continuous mixture with weights from the Dirichlet prior \\(p(\theta|\alpha)\\), which exhibits a multimodal structure when \\(\alpha<1\\). It can also be acquired by applying a change of variable to the prior pdf 
