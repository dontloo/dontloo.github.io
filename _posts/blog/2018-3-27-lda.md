...
### Latent Dirichlet
A document is a multinomial over topics, a topic is a multinomial over words, the distribution of words given a document \\(p(w|\theta,\beta) = \sum_zp(z|\theta)p(w|z,\beta)=Multi(w|\mu)\\), where \\(\mu=f_\beta(\theta)=\theta\beta\\) is a vector in the "word" space. As shown in the LDA paper, the distribution of \\(\mu\\) is continuous mixture with weights from the Dirichlet prior \\(p(\theta|\alpha)\\), which can also be acquired by applying a change of variable to the prior pdf \\(p(\mu|\alpha,\beta) = p(f_\beta(\theta)|\alpha)\\).  
