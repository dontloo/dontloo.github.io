...
In the collapsed Gibbs setting, it turns out the only information necessary for sampling from \\(p(\mathbf{z}|\mathbf{w})\\) are the document–topic counts \\(n_m\\) and the topic–term counts \\(n_k\\), which allows the algorithm to be implemented efficiently. Based on a full set of samples \\(\mathbf{z}\\), the distribution of \\(\beta\\) and \\(\theta\\) can be estimated by   
