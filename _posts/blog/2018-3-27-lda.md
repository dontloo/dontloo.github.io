...
This paper [Thomas L Griffiths and Mark Steyvers. Finding scientific topics. 2004.](http://www.pnas.org/content/101/suppl_1/5228.short) presents an extension of the original LDA model, which is to model \\(\beta\\) as a latent variable with another Dirichlet distribution \\(p(\phi)\\) as its prior. Then by treating \\(\alpha\\) and \\(\phi\\) as hyper-parameters, the paper uses collapsed Gibbs sampling to sample from the distribution \\(p(\mathbf{z}|\mathbf{w})\\).
