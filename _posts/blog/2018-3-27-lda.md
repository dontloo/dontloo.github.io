...
In the collapsed Gibbs setting, it turns out the only information necessary for sampling from \\(p(\mathbf{z}|\mathbf{w})\\) are the document–topic counts and the topic–term counts, which allows the algorithm to be implemented efficiently.
