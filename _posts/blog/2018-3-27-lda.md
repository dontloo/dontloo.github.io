...
For inference, collapsed Gibbs sampling can also be used to sample the latent variables \\(p(\mathbf{\tilde{z}}|\mathbf{\tilde{w}},\mathbf{z},\mathbf{w})\\), then \\(\beta\\) can be estimated by \\[\beta=E_{p(\mathbf{z}|\mathbf{w})}[p(\beta|\mathbf{w},\mathbf{z})]\approx p(\beta|\mathbf{w},\bar{\mathbf{z}}).\\]
