...
where \\( w \\) are model parameters and \\( \phi(x,y) \\) are called the features (or [sufficient statistics](https://stats.stackexchange.com/questions/44063/sufficient-statistics-for-layman)).
...
If the hidden variable is categorical, it often only serves as an indicator. For example a simple chain structured CRF can be written as
