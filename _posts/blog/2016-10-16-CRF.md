...
If the hidden variables \\(y\\) are categorical, it might seem a bit strange to make the "feature" depend on \\(y\\), so in such case \\(y\\) often serves as an indicator. For example a simple chain structured CRF can be written as
